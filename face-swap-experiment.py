...
# Image paths
SOURCE_IMAGE = "img/vsauce.jpg"  # Source face to swap from
TARGET_IMAGE = "img/image0.jpg"  # Target image to swap onto (when USE_WEBCAM = False)
...
if USE_WEBCAM:
    ...
    while True:
        ...
        if len(faces2) > 0:
            for face2 in faces2:
                ...
                if moments["m00"] != 0 and USE_SEAMLESS_CLONE:
    ...
    # Image input mode
    img2 = cv2.imread(TARGET_IMAGE)  # Change this to your target image
    img2_gray = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)
    img2_new_face = np.zeros_like(img2)
